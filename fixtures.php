...
//                    $query = "SELECT * FROM matches WHERE matchstatus='scheduled' AND g_id='1' ";
			$query     = "SELECT m.*, t1.team_name as `team_1_name`, t2.team_name as `team_2_name` FROM matches AS m INNER JOIN teams AS t1 ON (t1.team_id = m.team1_id) INNER JOIN teams AS t2 ON (t2.team_id = m.team2_id) WHERE m.matchstatus='scheduled'";
